...
// GitHub repo info
const GITHUB_OWNER = 'badri';
const GITHUB_REPO = 'wt';
...
async function install() {
  try {
    ...
    console.log(`Installing wt v${VERSION} for ${platformName}-${archName}...`);

    // Construct download URL
    // Format: https://github.com/badri/wt/releases/download/v0.1.0/wt_0.1.0_darwin_arm64.tar.gz
    const archiveExt = platformName === 'windows' ? 'zip' : 'tar.gz';
    const archiveName = `wt_${VERSION}_${platformName}_${archName}.${archiveExt}`;
    ...
    console.log(`Downloading wt binary...`);
    ...
    try {
      ...
      console.log(`wt installed successfully: ${output.trim()}`);
    } catch (err) {
      console.warn('Warning: Could not verify binary version');
    }

  } catch (err) {
    console.error(`Error installing wt: ${err.message}`);
    console.error('');
    console.error('Installation failed. You can try:');
    console.error(`1. Installing manually from: https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/releases`);
    console.error(`2. Using go install: go install github.com/${GITHUB_OWNER}/${GITHUB_REPO}/cmd/wt@latest`);
    console.error(`3. Opening an issue: https://github.com/${GITHUB_OWNER}/${GITHUB_REPO}/issues`);
    process.exit(1);
  }
}
